...
class TourPackageController extends Controller
{
    public function detail($slug)
    {
        ...
        $randomDestination = Destination::where('id', '<>', $destination->id)->where('top', 'no')->where('publish', 'yes')->inRandomOrder()->take('5')->get();
        return view('frontend.tour-package-detail', compact('destination', 'randomDestination'));
    }
    ...
    public function bookTourPackage(Request $request, $destinationId)
    {
        $data = $request->all();
    ...
    public function all()
    {
        $destinations = Destination::where('top', 'no')->where('publish', 'yes')->orderBy('id', 'DESC')->paginate('6');
        $count = Destination::where('top', 'no')->where('publish', 'yes')->count();
        return view('frontend.all-tour-packages', compact('destinations', 'count'));
    }
}
